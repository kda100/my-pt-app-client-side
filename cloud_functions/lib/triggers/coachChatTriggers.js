        ...
        if (typeof newChatDocData !== "undefined") {
            ...
            if (sentBy === coachId) {
                if (chatContentItemType == chatTextContentType) {
                    await firebaseMessagesHelper_1.FirebaseMessagesHelper.sendMessageToClient(clientId, "New Message", `Your Coach: ${content}`);
                }
                else {
                    await firebaseMessagesHelper_1.FirebaseMessagesHelper.sendMessageToClient(clientId, "New Message", "Your coach has sent you a new message");
                }
            }
                ...
                if (clientFullName !== undefined) {
                    if (chatContentItemType == chatTextContentType) {
                        await firebaseMessagesHelper_1.FirebaseMessagesHelper.sendMessageToCoach(coachId, "New Message", `${clientFullName}: ${content}`);
                    }
                    else {
                        await firebaseMessagesHelper_1.FirebaseMessagesHelper.sendMessageToCoach(coachId, "New Message", `${clientFullName} has sent you a new message`);
                    }
                }
                else {
                    await firebaseMessagesHelper_1.FirebaseMessagesHelper.sendMessageToCoach(coachId, "New Message", "You have a new message");
                }
            }
        }
    }
...
//     }
// });
//# sourceMappingURL=coachChatTriggers.js.map