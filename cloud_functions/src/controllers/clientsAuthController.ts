...
clientsAuthApp.post("/client_registration", ValidationHelper.userCreationValidators, async (req: express.Request, res: express.Response) => {
    let userUID;
...
clientsAuthApp.post("/check_client_login", ValidationHelper.userLoginValidators, async (req: express.Request, res: express.Response) => {
    const errors = validationResult(req).formatWith(ValidationHelper.authErrorFormatter);
...
clientsAuthApp.post("/create_client_from_user", ValidationHelper.userLoginValidators, async (req: express.Request, res: express.Response) => {
    const errors = validationResult(req).formatWith(ValidationHelper.authErrorFormatter);
    if (!errors.isEmpty()) {
        functions.logger.log(errors.array());
        return res.status(400).json({error: errors.array()});
