...
import {ExerciseAndMealPlanHelper} from "../helpers/exerciseAndMealPlanHelper";
            ...
            if (typeof afterDocData !== "undefined") {
                if (!snap.before.exists && snap.after.exists) {
                    ...
                    if (typeof beforeDocData !== "undefined") {
                        const mealAndExercisePlanIndex = ExerciseAndMealPlanHelper.getCompletedExerciseOrMealIndex(beforeDocData[fieldNames.completedArrField], afterDocData[fieldNames.completedArrField]);
...
async function onExerciseOrMealPlanAdded(docId: string, exerciseAndMealPlan: ExerciseAndMealPlan, afterDocData: FirebaseFirestore.DocumentData, coachId: string, clientId: string,) {
    ...
    const clientFullName: string = await ClientsHelper.getClientFullName(clientId);
    const planName: string = ExerciseAndMealPlanHelper.getExerciseOrMealPlanName(exerciseAndMealPlan);
    const dateString: string = TimeStampHelper.convertTimestampToDayMonthYear(afterDocData[fieldNames.dateField]);

    RecentUpdatesHelper.setRecentUpdateForClientUsingBatch(clientId, `Your coach has added a new ${planName} for ${dateString}.`, batch);
    RecentUpdatesHelper.setRecentUpdateForCoachUsingBatch(coachId, `You have added a new ${planName} for ${clientFullName}.`, batch);
    await FirebaseMessagesHelper.sendMessageToClient(clientId, `New ${planName} Added`, `Your coach has added a new ${planName} for ${dateString}.`);
...
async function onExerciseOrMealCompleted(docId: string, exerciseAndMealPlan: ExerciseAndMealPlan, afterDocData: FirebaseFirestore.DocumentData, coachId: string, clientId: string, mealOrExerciseIndex: number,) {
    ...
    const clientFullName: string = await ClientsHelper.getClientFullName(clientId);
    const planName: string = ExerciseAndMealPlanHelper.getExerciseOrMealPlanName(exerciseAndMealPlan);
    const exercisesOrMealsFieldName: string = ExerciseAndMealPlanHelper.getExerciseOrMealFieldName(exerciseAndMealPlan);
    const singularExerciseOrMeal: string = ExerciseAndMealPlanHelper.getSingularExerciseOrMeal(exerciseAndMealPlan);

    await updateClientPerformance(exerciseAndMealPlan, coachId, clientId);
    const timeString: string = TimeStampHelper.convertTimestampToHHMM(afterDocData[exercisesOrMealsFieldName][mealOrExerciseIndex][fieldNames.timeField]);
    const coachesMessage = `${clientFullName} has completed ${singularExerciseOrMeal} scheduled for ${timeString}.`;
    RecentUpdatesHelper.setRecentUpdateForCoachUsingBatch(coachId, coachesMessage, batch);
    RecentUpdatesHelper.setRecentUpdateForClientUsingBatch(clientId, `You have completed ${singularExerciseOrMeal} scheduled for ${timeString}.`, batch);

    await batch.commit();
    await FirebaseMessagesHelper.sendMessageToCoach(coachId, `${planName} Completed.`, coachesMessage);
...
async function onModifiedExerciseOrMealPlan(docId: string, exerciseAndMealPlan: ExerciseAndMealPlan, afterDocData: FirebaseFirestore.DocumentData, coachId: string, clientId: string,) {
    ...
    RecentUpdatesHelper.setRecentUpdateForClientUsingBatch(clientId, `Your coach has updated your ${planName} for ${dateString}.`, batch);
    RecentUpdatesHelper.setRecentUpdateForCoachUsingBatch(coachId, `You have updated the ${planName} for ${clientFullName} on ${dateString}.`, batch);
    await FirebaseMessagesHelper.sendMessageToClient(clientId, `${planName} Update`, `Your coach has updated your ${planName} for ${TimeStampHelper.convertTimestampToDayMonthYear(afterDocData[fieldNames.dateField])}.`);
...
async function onRemovedExcercisePlan(docId: string, exerciseAndMealPlan: ExerciseAndMealPlan, beforeDocData: FirebaseFirestore.DocumentData, coachId: string, clientId: string) {
    ...
    const dateString: string = TimeStampHelper.convertTimestampToDayMonthYear(beforeDocData[fieldNames.dateField]);
    RecentUpdatesHelper.setRecentUpdateForCoachUsingBatch(coachId, `You have removed a ${planName} for ${clientFullName} on ${dateString}.`, batch);
    RecentUpdatesHelper.setRecentUpdateForClientUsingBatch(clientId, `Your coach has removed a ${planName} for ${dateString}.`, batch);
