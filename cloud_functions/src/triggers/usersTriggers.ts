...
import {getClientsWithCoachUID} from "../helpers/clientsHelper";
...
exports.onDeleteUser = functions.auth.user().onDelete(async (user) => {
    ...
    try {
        const userDocSnapshotData: FirebaseFirestore.DocumentData | undefined = await getUserDocData(userUID);
        if (typeof userDocSnapshotData !== "undefined") {
            ...
            if (userDocSnapshotData[fieldNames.isClientField]) {
                ...
                const coachesDocsQuerySnapshot: FirebaseFirestore.QuerySnapshot<FirebaseFirestore.DocumentData> = await getCoachesWithClientUID(userUID);
            ...
            if (userDocSnapshotData[fieldNames.isCoachField]) {
                ...
                const clientsDocsQuerySnapshot: FirebaseFirestore.QuerySnapshot<FirebaseFirestore.DocumentData> = await getClientsWithCoachUID(user.uid);
