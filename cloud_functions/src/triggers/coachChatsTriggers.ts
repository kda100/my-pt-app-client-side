        ...
        if (typeof newChatDocData !== "undefined") {
            ...
            if (sentBy === coachId) {
                if (chatContentItemType == chatTextContentType) {
                    await sendMessageToClient(clientId, "New Message", `Your Coach: ${content}`);
                } else {
                    await sendMessageToClient(clientId, "New Message", "Your coach has sent you a new message");
                }
            } else if (sentBy === clientId) {
                const clientFullName: string | undefined = await getClientFullName(clientId);
                if (clientFullName !== undefined) {
                    if (chatContentItemType == chatTextContentType) {
                        await sendMessageToCoach(coachId, "New Message", `${clientFullName}: ${content}`);
                    } else {
                        await sendMessageToCoach(coachId, "New Message", `${clientFullName} has sent you a new message`);
                    }
                } else {
                    await sendMessageToCoach(coachId, "New Message", "You have a new message");
                }
            }
        }
    } catch (error) {
