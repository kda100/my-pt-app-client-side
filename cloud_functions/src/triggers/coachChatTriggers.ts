...
import {chatsMediaBucket} from "../services/firebaseServices";
...
exports.onCreateChatContentItem = functions.firestore.document(chatContentsDocRefString).onCreate(async (snap, context) => {
    try {
        const newChatDocData: FirebaseFirestore.DocumentData | undefined = snap.data();
        if (typeof newChatDocData !== "undefined") {
            ...
            const sentBy: string = newChatDocData[fieldNames.sentByField];
            const content: string = newChatDocData[fieldNames.contentField];
            const chatContentItemType: string = newChatDocData[fieldNames.chatContentItemTypeField];
            ...
            if (sentBy === coachId) {
                if (chatContentItemType == chatTextContentType) {
                    await FirebaseMessagesHelper.sendMessageToClient(clientId, "New Message", `Your Coach: ${content}`);
                } else {
                    await FirebaseMessagesHelper.sendMessageToClient(clientId, "New Message", "Your coach has sent you a new message");
                }
            } else if (sentBy === clientId) {
                const clientFullName: string | undefined = await ClientsHelper.getClientFullName(clientId);
                if (clientFullName !== undefined) {
                    if (chatContentItemType == chatTextContentType) {
                        await FirebaseMessagesHelper.sendMessageToCoach(coachId, "New Message", `${clientFullName}: ${content}`);
                    } else {
                        await FirebaseMessagesHelper.sendMessageToCoach(coachId, "New Message", `${clientFullName} has sent you a new message`);
                    }
                } else {
                    await FirebaseMessagesHelper.sendMessageToCoach(coachId, "New Message", "You have a new message");
                }
            }
        }
    } catch (error) {
...
exports.onDeleteChatContentItem = functions.firestore.document(chatContentsDocRefString).onDelete(async (snap, context) => {
    const coachId: string = context.params.coachId;
    const clientId: string = context.params.clientId;
    const chatContentItemId: string = context.params.chatContentItemId;
    ...
    try {
        ...
        if (typeof chatContentItemDocData !== "undefined") {
            ...
            if (chatContentItemType == "image") {
                const chatImageFile = chatsMediaBucket.file(`${coachId}/${clientId}/images/${chatContentItemId}`);
            ...
            if (chatContentItemType == "video") {
                await chatsMediaBucket.deleteFiles({prefix: `${coachId}/${clientId}/videos/${chatContentItemId}`});
            }
