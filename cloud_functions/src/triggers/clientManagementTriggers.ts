    ...
    try {
        if (coachDocAfter.exists && coachDocBefore.exists) {
            ...
            if (coachDocDataBefore[fieldNames.firstNameField] !== coachDocDataAfter[fieldNames.firstNameField]) {
                await RecentUpdatesHelper.setRecentUpdateForCoachClientsUsingBatch(coachId, "Your coach has changed their first name", batch);
            }
            if (coachDocDataBefore[fieldNames.lastNameField] !== coachDocDataAfter[fieldNames.lastNameField]) {
                await RecentUpdatesHelper.setRecentUpdateForCoachClientsUsingBatch(coachId, "Your coach has changed their last name", batch);
            }
            if (coachDocDataBefore[fieldNames.dateOfBirthField].toDate().getTime() !== coachDocDataAfter[fieldNames.dateOfBirthField].toDate().getTime()) {
                await RecentUpdatesHelper.setRecentUpdateForCoachClientsUsingBatch(coachId, "Your coach has changed their date of birth", batch);
            }
            if (coachDocDataBefore[fieldNames.phoneNumberField] !== coachDocDataAfter[fieldNames.phoneNumberField]) {
                await RecentUpdatesHelper.setRecentUpdateForCoachClientsUsingBatch(coachId, "Your coach has changed their phone number", batch);
            }
            if (coachDocDataBefore[fieldNames.profilePicDownloadURLField] !== coachDocDataAfter[fieldNames.profilePicDownloadURLField]) {
                await RecentUpdatesHelper.setRecentUpdateForCoachClientsUsingBatch(coachId, "Your coach have changed their profile picture", batch);
            }
        }
    } catch (error) {
