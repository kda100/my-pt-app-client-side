...
import {setRecentUpdateForCoachClientsUsingBatch, setRecentUpdateForCoachUsingBatch, setRecentUpdateForCoach} from "../helpers/recentUpdatesHelper";
import {sendMessageToCoachClients} from "../helpers/firebaseMessagesHelper";
...
exports.onCreateExerciseTutorial = functions.firestore.document(exerciseTutorialDocRefString).onCreate(async (snap, context) => {
    try {
        ...
        setRecentUpdateForCoachClientsUsingBatch(coachId, clientsMessage, batch);
        setRecentUpdateForCoachUsingBatch(coachId, "You have uploaded a new Exercise Tutorial", batch);
        await batch.commit();
        await sendMessageToCoachClients(coachId, "New Exercise Tutorial Uploaded", clientsMessage);
    } catch (error) {
...
exports.onCreatePosingTutorial = functions.firestore.document(posingTutorialDocRefString).onCreate(async (snap, context) => {
    try {
        ...
        setRecentUpdateForCoachUsingBatch(coachId, "You have uploaded a new Exercise Tutorial", batch);
        await batch.commit();
        await sendMessageToCoachClients(context.params.coachId, "New Posing Tutorial Uploaded", "Your coach has uploaded a new posing tutorial");
    } catch (error) {
        if (error instanceof Error) {
            functions.logger.log(error.message);
        } else {
...
exports.onDeleteExerciseTutorial = functions.firestore.document(exerciseTutorialDocRefString).onDelete(async (snap, context) => {
    try {
        await setRecentUpdateForCoach(context.params.coachId, "You have removed Exercise Tutorial");
...
exports.onDeletePosingTutorial = functions.firestore.document(posingTutorialDocRefString).onDelete(async (snap, context) => {
    try {
        await setRecentUpdateForCoach(context.params.coachId, "You have removed Posing Tutorial");
