...
export async function getCoachesWithClientUID(clientUID: string): Promise<firestore.QuerySnapshot<firestore.DocumentData>> {
...
export function getCoachProfilePicPath(coachUID: string): string {
    return `${coachUID}/${storageNames.profilePicFileName}`;
}

export async function getClientFullName(clientUID: string): Promise<string | undefined> {
