...
export const authErrorFormatter: ({param}: ValidationError) => string = ({param}: ValidationError) => {
...
export async function decodeFirebaseIdToken(firebaseIdToken: string | undefined): Promise<DecodedIdToken | Error> {
...
export const userCreationValidators = [body(fieldNames.emailField).notEmpty().isEmail(),
...
export const userLoginValidators = [body(fieldNames.emailField).notEmpty().isEmail()];

export const userUpdateValidators = [body(fieldNames.firstNameField).notEmpty().isLength({max: 50, min: 2}).matches(nameRegExString), //    eslint-disable-next-line
body(fieldNames.lastNameField).notEmpty().isLength({max: 50, min: 2}).matches(nameRegExString),
body(fieldNames.dateOfBirthField).notEmpty().custom(isValidAge),
body(fieldNames.phoneNumberField).optional({checkFalsy: true}).isMobilePhone("en-GB"),
body(fieldNames.companyNameField).optional({checkFalsy: true}).isLength({max: 50, min: 2}).matches(nameRegExString),
