...
coachesAuthApp.post("/coach_registration", userCreationValidators, async (req: express.Request, res: express.Response) => {
    let userUID;
    ...
    try {
        ...
        batch.set(db.collection(collectionNames.coaches).doc(userUID), {
            firstName: firstName,
            lastName: lastName,
        ...
        batch.set(db.collection(collectionNames.clientManagement).doc(userUID), {
            firstName: firstName,
            lastName: lastName,
            phoneNumber: phoneNumber,
            dateOfBirth: dateOfBirth,
...
coachesAuthApp.post("/check_coach_login", userLoginValidators, async (req: express.Request, res: express.Response) => {
    const errors = validationResult(req).formatWith(authErrorFormatter);
    if (!errors.isEmpty()) {
...
coachesAuthApp.post("/create_coach_from_user", userLoginValidators, async (req: express.Request, res: express.Response) => {
    const errors = validationResult(req).formatWith(authErrorFormatter);
    if (!errors.isEmpty()) {
        functions.logger.log(errors.array());
        return res.status(400).json({error: errors.array()});
        ...
        if (userDocSnapshot.exists) {
            ...
            if (typeof userDocSnapshotData !== "undefined") {
                if (!userDocSnapshotData[fieldNames.isCoachField]) {
                    ...
                    batch.set(db.collection(collectionNames.coaches).doc(user.uid), {
                    ...
                    batch.set(db.collection(collectionNames.clientManagement).doc(user.uid), {
                        firstName: userDocSnapshotData.firstName,
                        lastName: userDocSnapshotData.lastName,
                        phoneNumber: "",
                        dateOfBirth: userDocSnapshotData.dateOfBirth,
